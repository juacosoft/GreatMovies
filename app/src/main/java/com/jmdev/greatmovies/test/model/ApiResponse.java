...
import static com.jmdev.greatmovies.test.viewModel.MoviesViewModel.QUERY_EXHAUSTED;

import com.jmdev.greatmovies.response.CheckMovieApiKey;
...
public class ApiResponse <T>{
    ...
    public ApiResponse<T> create(Response<T> response){
        ...
        if(response.isSuccessful()){
            ...
            if(body instanceof MovieDBResponse){
                if(!CheckMovieApiKey.isMovieKeyValid((MovieDBResponse)body)){
                    String errorMsg = "Api key is invalid or expired.";
                    return new ApiErrorResponse<>(errorMsg);
                }
                if(((MovieDBResponse) body).getTotalMovies() == 0){
                    // query is exhausted
